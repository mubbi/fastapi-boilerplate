...
- **DX:** `make setup ENV=local|test|production` is the **single** environment entrypoint. **Lint, tests, i18n, and security tools** run only via **Docker** (`docker/Dockerfile.dev`, `docker compose -f docker-compose.test.yml run …`); see the Makefile. **Git hooks** (copy `docker/.githooks/` → `.git/hooks/` via `install_git_hooks.sh`) run **`make git-hooks-commit`** on commit (lint + `check-env`) and **`make git-hooks-push`** before push (same as **`make ci-local`** unless **`GIT_HOOKS_PUSH_QUICK=1`**; no host Python, no `pre-commit` package). Tests use an isolated stack (`docker-compose.test.yml`); never the dev DB.
